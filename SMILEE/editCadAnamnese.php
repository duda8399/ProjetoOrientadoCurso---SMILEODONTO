						...
						<div class="row">
							...
							<div class="col-lg-9">
								<div class="conteudo-form">
									...
									<form id="add-anamnese" method="POST" action="anamnese_controller.php?acao=atualizar" enctype="multipart/form-data">
										<div class="form-row form1" id="formulario">

											<?php foreach ($recuperar_questionario as $questionario) { ?>
												...
												<div class="col-lg-8">
													...
													<input type="hidden" name="idQuestionario" value="<?php echo $questionario['idQuestionario']; ?>">

													<input type="text" name="anamnese" id="anamnese" placeholder="Digite um nome" class="form-control" required="" value="<?php echo $questionario['nomeFicha']; ?>">
												</div>
											<?php } ?>
											...
											<?php foreach ($recuperar_Questao as $questao) { 
												$meualerta = $questao['idAlerta'];
												$minhaobg = $questao['idObrigatoriedade']; ?>

												<div class="col-lg-12 mt-4" id="removee">
													<div class="row">
														<i class="fas fa-trash" style="margin-top: 40px;margin-left: 5px;cursor: pointer;" id="remove"></i>

														<div class="col-lg-6">
															<label for="pergunta">Pergunta:</label>
															<input type="text" id="pergunta" name="questao[]" class="form-control" placeholder="Digite a pergunta" value="<?php echo $questao['enunciado'] ?>">
														</div>

														<div class="col-lg-2">
															<label for="obrigatorio">Obrigatoriedade:</label>
															<select class="form-control" name="obrigatorio[]" id="obrigatorio">
																<?php foreach ($recuperar_obg as $obg) { 
																	$obgprincipal = $obg['idObrigatoriedade']; 

																	if ($obgprincipal == $minhaobg) { ?>

																	<option value="<?php echo $obg["idObrigatoriedade"]; ?>" selected>
																		<?php echo $obg["obrigatorio"]; ?>	
																	</option>

																<?php }else { ?>
																	<option value="<?php echo $obg["idObrigatoriedade"]; ?>">
																		<?php echo $obg["obrigatorio"]; ?>	
																	</option>
																<?php }} ?>
															</select>
														...
														<div class="col-lg-3">
															...
															<select class="form-control" name="alerta[]" id="alerta">
																<?php foreach ($recuperar_alerta as $alerta) { 
																	$alertaprincipal = $alerta['idAlerta'];

																	if ($meualerta == $alertaprincipal) { ?>

																	<option value="<?php echo $alerta["idAlerta"]; ?>" selected>
																		<?php echo $alerta["alerta"]; ?>	
																	</option>
																<?php }else{ ?>
																	<option value="<?php echo $alerta["idAlerta"]; ?>">
																		<?php echo $alerta["alerta"]; ?>	
																	</option>
																<?php }} ?>
															</select>
