<?php
	...
	while( $row_pacientes =mysqli_fetch_array($resultado_pacientes) ) {  
		$dado = array(); 
		$dado[] = '<a class="btn btn-outline-light btn-custom" href="listPacienteS.php?id='.$row_pacientes["idPessoa"].'"><span><i class="fas fa-check"></i></span></a>';
		$dado[] = $row_pacientes["nome"];
		$dado[] = $row_pacientes["logradouro"];
		$dado[] = $row_pacientes["telefoneCelular"];
		$dado[] = '<button type="submit" name="editar" class="btn btn-editar text-white mr-2" value="'.$row_pacientes["idPessoa"].'">Editar</button>'.'<button type="button" name="excluir" id="excluir" class="btn btn-excluir text-white" value="'.$row_pacientes["idPessoa"].'" >Excluir</button>';	
		$dados[] = $dado;
	}
