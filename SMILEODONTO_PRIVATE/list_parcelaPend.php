<?php
	...
	$result_parcela = "SELECT p.nome, trat.idTratamento, f.valorParcela, t.telefoneCelular, f.dataVencimento, s.status FROM pessoa p, financeiro f, statuspagamento s, tratamento trat, telefonepessoa t WHERE f.idTratamento = trat.idTratamento AND trat.idPaciente = p.idPessoa AND t.idPessoa = p.idPessoa AND s.idStatusPagamento = 1 AND f.idStatusPagamento = s.idStatusPagamento ";
	$resultado_parcela =mysqli_query($conecta, $result_parcela);
	$qnt_linhas = mysqli_num_rows($resultado_parcela);


	//Obter os dados a serem apresentados
	$result_parcelas = "SELECT p.nome, trat.idTratamento, f.valorParcela, t.telefoneCelular, f.dataVencimento, s.status FROM pessoa p, financeiro f, statuspagamento s, tratamento trat, telefonepessoa t WHERE f.idTratamento = trat.idTratamento AND trat.idPaciente = p.idPessoa AND t.idPessoa = p.idPessoa AND s.idStatusPagamento = 1 AND f.idStatusPagamento = s.idStatusPagamento ";
