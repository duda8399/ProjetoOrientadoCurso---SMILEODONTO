	...
	<?php 
		...
		if ($acao == 'recuperar') {
			...
			if(!$recuperar_proced) {
			    header("Location: listOrcamentoS.php?msg=5");
			}
			...
			if (isset($_GET['id'])) {
			    ...
			    if(!$remover_procedtrat) {
			        header("Location: cadOrcamentoS.php?msg=5");
			   	}else{
			   		header("Location: cadOrcamentoS.php?msg=3");
			   	}
			}
		        ...
		        for($i = 0; $i< count($valorParcela); $i++){
		        	...
		        	if (!empty($aumento)) {
						...
						header("Location: cadOrcamentoS.php?msg=5");
							...
							header("Location: listOrcamentoS.php?msg=8");
						}
